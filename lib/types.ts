...
export interface ChatMessage {
  ...
  error?: ChatError
  /**
   * Suggested next questions, generated by the backend from this turn's Q/A
   * and pushed on the `follow_up` SSE event after `done`.
   *
   * Only ever persisted on the newest assistant message — `syncToBackend`
   * strips it from the rest, so older turns shed theirs with no migration.
   * Absent when generation failed or was skipped; the UI falls back to its
   * own pool (see `FALLBACK_FOLLOW_UPS`).
   */
  followUps?: string[]
  /**
